...
export default function AppDashboard({ onNavigate }: AppDashboardProps) {
  ...
  const [reports, setReports] = useState<Report[]>([
    {
      ...
      modules: {
        "source-a": { url: "https://openai.com/blog", keywords: "GPT, AI models", viewpoint: "Technical", summary: sampleSummaries[0], isConfigured: true },
        "source-b": { url: "https://techcrunch.com/ai", keywords: "AI startups", viewpoint: "Market", summary: sampleSummaries[1], isConfigured: true },
    ...
    },
    {
      ...
      modules: {
        "source-a": { url: "https://bloomberg.com", keywords: "Q4 earnings", viewpoint: "Financial", summary: sampleSummaries[0], isConfigured: true },
  ...
  const currentReport = currentReportId ? reports.find(r => r.id === currentReportId) : null
  ...
  const [modalForm, setModalForm] = useState({ url: "", keywords: "", viewpoint: "" })
  const [isGenerating, setIsGenerating] = useState(false)
  const [integratedResult, setIntegratedResult] = useState<string | null>(currentReport?.integratedResult || null)
  ...
  const configuredCount = Object.values(modules).filter(m => m.isConfigured).length
  ...
  const updateCurrentReport = (newModules: Record<ModuleId, ModuleConfig>, newIntegratedResult?: string | null) => {
    if (currentReportId) {
      setReports(prev => prev.map(r => 
        r.id === currentReportId 
          ? { ...r, modules: newModules, integratedResult: newIntegratedResult ?? r.integratedResult }
          : r
      ))
    }
  }
  ...
  const handleSaveAndGenerate = async () => {
    ...
    const moduleIndex = ["source-a", "source-b", "source-c"].indexOf(activeModal)
  ...
  const moduleLabels: Record<ModuleId, { label: string; color: string }> = {
    "source-a": { label: "Source A", color: "bg-emerald-100 text-emerald-700" },
    "source-b": { label: "Source B", color: "bg-blue-100 text-blue-700" },
    "source-c": { label: "Source C", color: "bg-amber-100 text-amber-700" },
  }
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform border-r border-border bg-sidebar transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold text-sidebar-foreground">RFM</span>
            </div>
            <button 
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex w-full items-center gap-3 rounded-lg p-2 transition-colors hover:bg-sidebar-accent">
                  ...
                  <div className="flex-1 text-left">
                    <p className="text-sm font-medium text-sidebar-foreground">gunho</p>
                    <p className="text-xs text-muted-foreground">교육 플러스 요금제 · 1명의 멤버</p>
                  </div>
                ...
                <Tabs defaultValue="settings" className="w-full">
                  ...
                  <TabsContent value="settings" className="m-0">
                    ...
                    <div className="border-b border-border px-3 py-2">
                      <p className="text-xs text-muted-foreground">eh5890@soongsil.ac.kr</p>
                    </div>
                      ...
                      {workspaceMembers.map((member, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            {member.isGuest && (
                              <span className="rounded bg-muted px-1.5 py-0.5 text-xs text-muted-foreground">게스트</span>
                            )}
                          </div>
                          {member.isOwner && <Check className="h-4 w-4 text-foreground" />}
                        </div>
                      ))}
                    </div>
        ...
        <div className="p-4 lg:p-8">
          ...
          <div className="mb-8">
            ...
            <p className="mt-1 text-muted-foreground">
              {currentReport 
                ? `Created on ${currentReport.createdAt.toLocaleDateString("ko-KR", { year: "numeric", month: "long", day: "numeric" })}`
          ...
          {!currentReportId ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h2 className="text-xl font-semibold text-foreground mb-2">No Report Selected</h2>
              <p className="text-muted-foreground mb-6 max-w-md">
                Create a new report to start researching, or select an existing one from the history.
              </p>
            ...
            <div className="grid gap-4 lg:grid-cols-2 lg:gap-6">
              ...
              {(["source-a", "source-b", "source-c"] as ModuleId[]).map((moduleId) => {
                const module = modules[moduleId]
                ...
                return (
                  ...
                  >
                    {/* Label Badge */}
                    <div className={`mb-4 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${color}`}>
                      {label}
                    </div>
                    ...
                    {module.isConfigured ? (
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 space-y-1">
                            <p className="text-sm text-muted-foreground line-clamp-1">
                              <span className="font-medium">URL:</span> {module.url}
                            </p>
                            <p className="text-sm text-muted-foreground line-clamp-1">
                              <span className="font-medium">Keywords:</span> {module.keywords}
                            </p>
                          </div>
                          <Button 
                            size="sm" 
                        ...
                        <div className="rounded-lg border border-border bg-muted/50 p-4">
                          ...
                          <p className="text-sm text-muted-foreground line-clamp-4">{module.summary}</p>
                        </div>
                      </div>
                    ) : (
                      /* Empty State */
                      <div className="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <p className="text-sm font-medium text-muted-foreground">Add Configuration</p>
                        <p className="mt-1 text-xs text-muted-foreground/70">Click to configure this source</p>
                      </div>
                    )}
                  </div>
                )
              ...
              <div className={`relative min-h-[200px] rounded-xl border bg-card p-6 shadow-sm lg:min-h-[240px] ${
                ...
                {integratedResult ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-primary" />
                        <span className="text-sm font-medium text-foreground">Synthesis Complete</span>
                      </div>
                      <Button 
                        size="sm" 
                        variant="ghost"
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-sm font-medium text-foreground">Integrated Perspective</p>
                    <p className="mt-1 mb-4 text-xs text-muted-foreground">
                      {configuredCount === 0 
                        ? "Configure at least one source to enable synthesis" 
                        : `${configuredCount} source${configuredCount > 1 ? "s" : ""} configured`
                      }
                    </p>
                    <Button
                      disabled={configuredCount === 0 || isGeneratingIntegrated}
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                Configure {activeModal && moduleLabels[activeModal].label}
              </DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-4 py-4">
              ...
              {/* Viewpoint */}
              <div className="space-y-2">
                <Label htmlFor="viewpoint" className="flex items-center gap-2 text-sm">
                ...
                <Textarea
                  ...
                  onChange={(e) => setModalForm(prev => ({ ...prev, viewpoint: e.target.value }))}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        I will read <span className="font-medium text-foreground">{modalForm.url}</span> and analyze it
                        {modalForm.keywords && <> focusing on <span className="font-medium text-foreground">{modalForm.keywords}</span></>}
                        {modalForm.viewpoint && <> from your specified viewpoint</>}.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex h-full">
              ...
              <div className="w-60 border-r border-border bg-muted/30 flex flex-col">
                ...
                <div className="p-3 border-b border-border">
                  <p className="text-xs text-muted-foreground mb-2">계정</p>
                  ...
                  <button className="flex items-center gap-2 w-full p-2 rounded-md bg-muted/70 text-sm text-foreground">
                    <Settings className="h-4 w-4" />
                    기본 설정
                  </button>
                  <button className="flex items-center gap-2 w-full p-2 rounded-md hover:bg-muted/50 text-sm text-foreground">
                ...
                <div className="p-3 flex-1 overflow-y-auto">
                  <p className="text-xs text-muted-foreground mb-2">워크스페이스</p>
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                <div className="p-6 pt-8">
                  <h2 className="text-lg font-semibold text-foreground mb-6">기본 설정</h2>
                  ...
                  <div className="mb-8">
                    ...
                    <p className="text-xs text-muted-foreground">내 기기에서 Notion의 모습을 마음껏 바꿔보세요.</p>
                  </div>
                  
                  {/* Language & Time Section */}
                  <div className="mb-6">
                    <h3 className="text-sm font-medium text-foreground mb-4 pb-2 border-b border-border">언어 및 시간</h3>
                    
                    {/* Language */}
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-foreground">언어</p>
                        <p className="text-xs text-muted-foreground">Notion에서 사용하는 언어를 변경하세요.</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-foreground">텍스트 방향 제어 항상 표시</p>
                        <p className="text-xs text-muted-foreground">언어가 힌족에서 오른쪽으로 표시되는 경우에도 편집기 메뉴에 텍스트 방향(LTR/RTL)을 변경하는 옵션을 표시합니다.</p>
                      </div>
                      <Switch checked={textDirection} onCheckedChange={setTextDirection} />
                    </div>
                    
                    {/* Week Start */}
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-foreground">한 주의 시작을 월요일로 설정하기</p>
                        <p className="text-xs text-muted-foreground">Notion의 모든 캘린더에 적용됩니다.</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-foreground">위치를 사용하여 자동으로 시간대 설정</p>
                        <p className="text-xs text-muted-foreground">설정한 시간대를 기준으로 리마인더, 알림, 이메일을 보내드려요.</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">현재 설정된 시간대입니다.</p>
                      </div>
                      <Select value={timezone} onValueChange={setTimezone}>
                        ...
                        <SelectContent>
                          <SelectItem value="(GMT+09:00) 서울">(GMT+09:00) 서울</SelectItem>
                          <SelectItem value="(GMT+00:00) UTC">(GMT+00:00) UTC</SelectItem>
                          <SelectItem value="(GMT-08:00) LA">(GMT-08:00) LA</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                  ...
                  {/* Desktop App Section */}
                  <div>
                    <h3 className="text-sm font-medium text-foreground mb-4 pb-2 border-b border-border">데스크톱 앱</h3>
                    
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-foreground">데스크톱 앱에서 링크 열기</p>
                        <p className="text-xs text-muted-foreground">Windows 앱이 설치되어 있어야 합니다.</p>
                      </div>
                      <Switch checked={desktopLinks} onCheckedChange={setDesktopLinks} />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Select defaultValue="last">
                        ...
                        <SelectContent>
                          <SelectItem value="last">마지막으로 본 페이지</SelectItem>
